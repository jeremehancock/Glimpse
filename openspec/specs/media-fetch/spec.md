# media-fetch Specification

## Purpose

Talking to Plex, Jellyfin and Emby, and turning what comes back into the
snapshot on the mounted volume that everything else in this project reads:
libraries, items, the JSON schema of `movies.json` and `tvshows.json`, and the
cron schedule that refreshes them.

Two properties shape the whole capability. **The fetchers are batch jobs, not
services** — cron runs them, `print()` is their interface, and `docker logs` is
how a user watches an import. And **the snapshot they write is being served
while they write it**, so a fetcher has to be correct at every moment of a run
rather than only at the end of one. That is why publishing is a rename over a
complete file and never an edit in place.

Emby and Jellyfin are one adapter with two identities; their APIs are
compatible, which is why a single fetcher serves both.
