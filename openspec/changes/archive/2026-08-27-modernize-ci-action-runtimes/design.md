...
| Risk | Mitigation |
| --- | --- |
| **The bumped `docker-publish.yml` cannot be exercised before the merge, and a `dev` push does not do it.** Confirmed on the run for `049ffc5`: it executed `actions/checkout@v4` and emitted the original six-action warning. `workflow_run` reads the workflow *file* from the default branch — the note at the top of that file says so about `github.ref`/`github.sha`, and it is equally true of the file's own contents. So the version bumps in it are inert until `main` carries them | Accept and state it. The `dev` run still proves the *build* works; what it cannot prove is that the new action versions do. The first real exercise is the merge to `main`. Watch that run, and expect the warning to disappear from the publish log only then |
